        ...
        return $table
            ...
            ->columns([
                Tables\Columns\TextColumn::make('name')->label('الاسم')->searchable()->weight('bold')
                    ->description(fn (Member $m) => $m->telegram_username ? '@' . $m->telegram_username : null),
                Tables\Columns\TextColumn::make('phone')->label('الهاتف')->placeholder('—')->toggleable(),
                Tables\Columns\TextColumn::make('listings_count')->label('إعلاناته')->counts('listings')->badge()->color('info'),
                Tables\Columns\TextColumn::make('tier')->label('الباقة')->badge()
                    ->formatStateUsing(fn ($s) => match ($s) { 'premium' => '⭐ مميّز', 'merchant' => '🏪 تاجر', default => 'عادي' })
                    ->color(fn ($s) => $s === 'none' ? 'gray' : 'warning'),
                Tables\Columns\TextColumn::make('status')->label('الحالة')->badge()
                    ->colors(['success' => 'active', 'danger' => 'banned'])
                    ->formatStateUsing(fn ($s) => $s === 'banned' ? 'محظور' : 'مفعّل'),
            ...
            ->filters([
                ...
                Tables\Filters\SelectFilter::make('tier')->label('الباقة')
                    ->options(['none' => 'عادي', 'premium' => 'مميّز', 'merchant' => 'تاجر']),
            ])
            ->actions([
                Tables\Actions\Action::make('toggle_ban')
                    ->label(fn (Member $m) => $m->status === 'banned' ? 'تفعيل' : 'حظر')
                    ->icon(fn (Member $m) => $m->status === 'banned' ? 'heroicon-o-check-circle' : 'heroicon-o-no-symbol')
                    ->color(fn (Member $m) => $m->status === 'banned' ? 'success' : 'danger')
                    ->requiresConfirmation()
                    ->action(fn (Member $m) => $m->update(['status' => $m->status === 'banned' ? 'active' : 'banned'])),
